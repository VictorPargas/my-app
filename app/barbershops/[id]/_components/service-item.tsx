...
const ServiceItem = ({ service, barbershop, isAuthenticates }: ServiceItemProps) => {
    ...
    const timeList = useMemo(() => {
      ...
      return generateDayTimeList(date).filter(time => {
        ...
        const booking = dayBookings.find(booking => {
            ...
            const bookingMunutes = booking.date.getMinutes();

            return bookingHour == timeHour && bookingMunutes == timeMinutes;
        })
                                     ...
                                     mode="single"
                                      selected={date}
                                       onSelect={handleDateClick}
                                        ...
                                        {date && (
                                            <div className=" flex gap-3  overflow-x-auto py-6 px-5 border-t border-secondary [&::-webkit-scrollbar]:hidden">
                                                {timeList.map((time) => (
                                                    <Button onClick={() => handleHourClick(time)} variant={hour == time ? "default" : "outline"}
